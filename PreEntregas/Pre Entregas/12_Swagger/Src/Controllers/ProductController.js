...
route.get('/mockingproducts', async (request, response, next) => {
    try {
        let { limit, page, sort, ...query } = request.query;
...
route.post('/', /* validateUserLevel(["admin"]) ,*/ async (request, response, next) => {
...
route.put('/:pid', /* validateUserLevel(["admin"]) ,*/ async (request, response, next) => {
    try {
        if (!request.params.pid) {
            throw new ValidationException("El valor ingresado no puede ser null");
        }
...
route.delete('/:pid', /* validateUserLevel(["admin"]) ,*/ async (request, response, next) => {
    try {
